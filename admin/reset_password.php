...
// Enhanced debugging
error_log("==== ADMIN RESET PASSWORD ATTEMPT ====");
error_log("Admin token received: " . $token);
...
if ($token) {
    ...
    if ($reset) {
        ...
        error_log("Admin token found in database for email: " . $email);
        ...
        if ($admin_user) {
            ...
            if (strtotime($expires_at) > time()) {
                $valid_token = true;
                error_log("Admin token is still valid");
            } else {
                $token_expired = true;
                error_log("Admin token has expired. Expired at: " . $expires_at);
            ...
            error_log("Token email is not associated with an admin account: " . $email);
        }
    } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && $_SERVER['HTTP_X_REQUESTED_WITH'] === 'XMLHttpRequest') {
    ...
    if (!$admin_user) {
        echo json_encode(['status' => 'error', 'message' => 'This reset link is not valid for admin accounts']);
        exit;
    }
    ...
    <div class="form-container animate-fade-in bg-white p-0 rounded-2xl shadow-2xl w-full max-w-xl overflow-hidden">
        ...
        <div class="p-8 bg-white">
            <div class="text-center mb-6">
                <h1 class="text-3xl font-bold text-gray-800 mb-2">Admin Reset Password</h1>
                <?php if ($valid_token): ?>
            ...
            <?php else: ?>
                <div class="text-center">
                    ...
                    <a href="forgot_password.php" class="inline-block bg-primary-600 hover:bg-primary-700 text-white font-semibold py-2 px-4 rounded-lg transition duration-300" id="requestNewLink">
                        Request New Link
                    </a>
                </div>
            <?php endif; ?>
