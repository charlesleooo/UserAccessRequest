...
// Ensure a Superior is logged in
...
error_log("Superior Create Request - Session data: " . json_encode($_SESSION));
error_log("Superior Create Request - Employee ID: " . $requestorId);
error_log("Superior Create Request - Username: " . $username);
...
try {
    ...
    if ($requestorInfo) {
        ...
        error_log("Superior Create Request - Retrieved user info: " . json_encode($requestorInfo));
    } else {
        error_log("Superior Create Request - No employee record found for ID: " . $requestorId);
        $fullName = $username;
        $company = '';
        $departmentName = '';
    }
} catch (PDOException $e) {
    error_log("Superior Create Request - Error fetching requestor info: " . $e->getMessage());
    $fullName = $username;
    $company = '';
    $departmentName = '';
}
                        ...
                        <table id="individualTable" class="w-full text-sm text-left text-gray-700">
                            <thead class="text-xs text-white uppercase bg-gradient-to-r from-blue-700 to-blue-900 border-b-2 border-blue-950">
                                <tr>
                                    ...
                                    <th scope="col" class="px-2 py-3 font-semibold w-[4%] ind-start-date-header hidden">Start Date <span class="text-red-600">*</span></th>
                                    <th scope="col" class="px-2 py-3 font-semibold w-[4%] ind-end-date-header hidden">End Date <span class="text-red-600">*</span></th>
                        ...
                        <table id="groupTable" class="w-full text-sm text-left text-gray-700">
                            <thead class="text-xs text-white uppercase bg-gradient-to-r from-blue-700 to-blue-900 border-b-2 border-blue-950">
                                <tr>
                                    ...
                                    <th scope="col" class="px-2 py-3 font-semibold w-[3%] grp-start-date-header hidden">Start Date <span class="text-red-600">*</span></th>
                                    <th scope="col" class="px-2 py-3 font-semibold w-[3%] grp-end-date-header hidden">End Date <span class="text-red-600">*</span></th>
    ...
    <script>
        ...
        function toggleDateFields(select) {
            ...
            if (isTemporary) {
                ...
                // Check if any other rows in this table still have temporary selected
                const allRows = table.querySelectorAll('tbody tr');
                let hasTemporary = false;
                allRows.forEach(r => {
                    const durationSelect = r.querySelector('.duration-select');
                    if (durationSelect && durationSelect.value === 'temporary') {
                        hasTemporary = true;
                    }
                });

                // Only hide headers if no rows have temporary selected
                if (!hasTemporary) {
                    if (isIndividualTable) {
        ...
        // Set minimum date for date-needed fields
        const today = new Date().toISOString().split('T')[0];
        ...
        // Company and Department fields are now auto-filled with user information
        ...
        $('#uarForm').on('reset', function(e) {
            ...
            const originalCompany = $('input[name="company"]').val();
            ...
            $('input[name="company"]').val(originalCompany);
            $('input[name="department"]').val(originalDepartment);

            // Reset all date field cells to hidden state
            ...
            // Hide all date headers
            $('.ind-start-date-header, .ind-end-date-header').addClass('hidden');
            $('.grp-start-date-header, .grp-end-date-header').addClass('hidden');
