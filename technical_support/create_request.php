...
// Ensure a Technical Support user is logged in
if (!isset($_SESSION['admin_id']) || ($_SESSION['role'] ?? '') !== 'technical_support') {
    header('Location: ../admin/login.php');
    exit();
}

// Mark verification flags (skip encryption prompt if any)
...
try {
    ...
    if ($info) {
        $fullName = $info['employee_name'] ?? $username;
    ...
    error_log('[TS Create Request] fetch info error: ' . $e->getMessage());
    $fullName = $username;
    $company = '';
    $department = '';
}
...
<head>
    <meta charset="UTF-8" />
    <title>Create Request - Technical Support</title>
...
<body class="bg-gray-100" x-data="{ sidebarOpen: true }" x-init="sidebarOpen = window.innerWidth >= 1024; window.addEventListener('resize', () => sidebarOpen = window.innerWidth >= 1024);">
    ...
    <div class="transition-all duration-300" :class="sidebarOpen ? 'md:ml-72' : 'ml-0'">
        <div class="bg-gradient-to-r from-blue-700 to-blue-900 border-b border-blue-800 sticky top-0 z-10 shadow-lg">
            <div class="flex items-center px-4 md:px-8 py-4 gap-3">
                <button @click="sidebarOpen=!sidebarOpen" type="button" class="inline-flex items-center justify-center rounded-lg p-2 text-white hover:bg-blue-800 focus:outline-none focus:ring-2 focus:ring-blue-300 mr-3 transition" aria-label="Toggle sidebar">
                    <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                        <path clip-rule="evenodd" fill-rule="evenodd" d="M2 4.75A.75.75 0 012.75 4h14.5a.75.75 0 010 1.5H2.75A.75.75 0 012 4.75zm0 10.5a.75.75 0 01.75-.75h7.5a.75.75 0 010 1.5h-7.5a.75.75 0 01-.75-.75zM2 10a.75.75 0 01.75-.75h14.5a.75.75 0 010 1.5H2.75A.75.75 0 012 10z"></path>
                    </svg>
                </button>
                <h2 class="text-2xl md:text-3xl font-bold text-white">Create New User Access Request (UAR)</h2>
            </div>
        </div>
            ...
            <form id="uarForm" class="w-full">
                        ...
                        <table class="w-full text-sm text-left text-gray-700 table-fixed">
                            <tbody>
                                <tr class="bg-white border-b hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 w-1/3"><input type="text" name="name" value="<?php echo htmlspecialchars($fullName); ?>" class="bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg w-full p-2.5 cursor-not-allowed" readonly /></td>
                                    <td class="px-6 py-4 font-semibold text-gray-900 bg-gray-50 w-1/6">Date</td>
                                    <td class="px-6 py-4 w-1/3"><input type="text" name="date" value="<?php echo date('Y-m-d'); ?>" class="bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg w-full p-2.5 cursor-not-allowed" readonly /></td>
                                </tr>
                                <tr class="bg-white border-b hover:bg-gray-50">
                                    <td class="px-6 py-4 font-semibold text-gray-900 bg-gray-50">Company</td>
                                    <td class="px-6 py-4"><input type="text" name="company" id="company" value="<?php echo htmlspecialchars($company); ?>" class="bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg w-full p-2.5 cursor-not-allowed" readonly /><input type="hidden" name="company_value" value="<?php echo htmlspecialchars($company); ?>" /></td>
                                    <td class="px-6 py-4 font-semibold text-gray-900 bg-gray-50">Department</td>
                                    <td class="px-6 py-4"><input type="text" name="department" id="department" value="<?php echo htmlspecialchars($department); ?>" class="bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg w-full p-2.5 cursor-not-allowed" readonly /><input type="hidden" name="department_value" value="<?php echo htmlspecialchars($department); ?>" /></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                ...
                <div class="flex items-center gap-4 p-4 mb-6 bg-gradient-to-r from-blue-700 to-blue-900 rounded-lg shadow-md border-b-4 border-blue-950">
                    ...
                    <div class="flex gap-2 items-center">
                        <label class="relative flex items-center px-3 py-1.5 bg-white border-2 border-gray-200 rounded cursor-pointer hover:border-blue-500 transition has-[input:checked]:border-blue-600 has-[input:checked]:ring-1 has-[input:checked]:ring-blue-300 has-[input:checked]:bg-blue-50">
                            <input type="radio" name="access_type" value="individual" checked class="sr-only" />
                            <div class="flex items-center gap-2"><svg class="w-4 h-4 text-blue-600" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clip-rule="evenodd" />
                                </svg><span class="text-xs font-semibold text-gray-900">Individual</span></div>
                        </label>
                        <label class="relative flex items-center px-3 py-1.5 bg-white border-2 border-gray-200 rounded cursor-pointer hover:border-blue-500 transition has-[input:checked]:border-blue-600 has-[input:checked]:ring-1 has-[input:checked]:ring-blue-300 has-[input:checked]:bg-blue-50">
                            <input type="radio" name="access_type" value="group" class="sr-only" />
                            <div class="flex items-center gap-2"><svg class="w-4 h-4 text-blue-600" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M13 6a3 3 0 11-6 0 3 3 0 016 0zM18 8a2 2 0 11-4 0 2 2 0 014 0zM14 15a4 4 0 00-8 0v3h8v-3zM6 8a2 2 0 11-4 0 2 2 0 014 0zM16 18v-3a5.972 5.972 0 00-.75-2.906A3.005 3.005 0 0119 15v3h-3zM4.75 12.094A5.973 5.973 0 004 15v3H1v-3a3 3 0 013.75-2.906z" />
                                </svg><span class="text-xs font-semibold text-gray-900">Group</span></div>
                        </label>
                    </div>
                </div>
                <!-- Individual Table -->
                <div id="individualAccess" class="mb-0">
                    <button type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-5 inline-flex items-center transition shadow-sm hover:shadow-md" onclick="addRow('individualTable')">
                        ...
                        <table id="individualTable" class="w-full text-sm text-left text-gray-700">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="px-4 py-3"><input type="text" placeholder="User Name" name="ind_user_names" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg w-full p-2.5" /></td>
                                    <td class="px-4 py-3">
                                        <select name="ind_application[]" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg w-full p-2.5" required>
                                            <option value="">Select Application</option>
                                            <?php foreach ($systemApplications as $app): ?><option value="<?php echo $app; ?>"><?php echo $app; ?></option><?php endforeach; ?>
                                        </select>
                                    ...
                                    <td class="border border-gray-200 p-1 hidden"><input type="date" name="ind_start_date[]" class="start-date w-full p-1 border border-gray-300 rounded" disabled /></td>
                                    <td class="border border-gray-200 p-1 hidden"><input type="date" name="ind_end_date[]" class="end-date w-full p-1 border border-gray-300 rounded" disabled /></td>
                                    <td class="border border-gray-200 p-1"><input type="date" name="ind_date_needed[]" class="date-needed w-full p-1 border border-gray-300 rounded" required /></td>
                                    <td class="border border-gray-200 p-2"><input type="text" placeholder="Click to add justification" name="ind_justification[]" class="justification-input w-full p-2 border border-gray-300 rounded bg-gray-50 cursor-pointer" required /></td>
                                    <td class="border border-gray-200 p-2 text-center w-16"><button type="button" class="bg-red-600 hover:bg-red-700 text-white p-2 rounded" onclick="deleteRow(this)"><i class="fa fa-trash"></i></button></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                ...
                <div id="groupAccess" class="mb-0 hidden">
                    <button type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-5 inline-flex items-center transition shadow-sm hover:shadow-md" onclick="addRow('groupTable')">
                        <svg class="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z" clip-rule="evenodd" />
                        </svg>
                        Additional Request
                        ...
                        <table id="groupTable" class="w-full text-sm text-left text-gray-700">
                            <thead class="text-xs text-white uppercase bg-gradient-to-r from-blue-700 to-blue-900 border-b-2 border-blue-950">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 font-semibold w-[32%]">User Name <span class="text-red-600">*</span></th>
                                    <th class="px-4 py-3 font-semibold w-[8%]">Access Type <span class="text-red-600">*</span></th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr class="application-row hover:bg-gray-50 transition-colors" data-app-id="1">
                                    <td class="px-4 py-3">
                                        <div class="flex gap-2 items-center"><select name="grp_application[]" class="app-select flex-1 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg w-full p-2.5" required>
                                                <option value="">Select Application</option><?php foreach ($systemApplications as $app): ?><option value="<?php echo $app; ?>"><?php echo $app; ?></option><?php endforeach; ?>
                                            </select><input type="hidden" name="grp_application_system[]" value="" /></div>
                                    </td>
                                    <td class="border border-gray-200 p-2">
                                        <div class="flex items-center gap-2"><input type="text" placeholder="User Name" name="grp_user_names[]" class="w-full p-2 border border-gray-300 rounded" required /><button type="button" class="bg-blue-600 hover:bg-blue-700 text-white p-2 rounded" onclick="addUserRow(this)" title="Add User"><i class="fa fa-user-plus"></i></button></div>
                                    </td>
                                    <td class="border border-gray-200 p-2"><select name="grp_access_type[]" class="w-full p-2 border border-gray-300 rounded" required>
                                            <option value=""></option>
                                            <option value="full">Full</option>
                                            <option value="read">Read</option>
                                            <option value="admin">Admin</option>
                                        </select></td>
                                    <td class="border border-gray-200 p-2"><select name="grp_duration_type[]" class="duration-select w-full p-2 border border-gray-300 rounded" required onchange="toggleDateFields(this)">
                                            <option value=""></option>
                                            <option value="permanent">Permanent</option>
                                            <option value="temporary">Temporary</option>
                                        </select></td>
                ...
                <div class="mt-8 flex justify-center gap-4 flex-wrap">
                    <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-base px-8 py-3.5 inline-flex items-center justify-center min-w-[200px] shadow-md hover:shadow-lg transition"><svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
                            <path d="M10.894 2.553a1 1 0 00-1.788 0l-7 14a1 1 0 001.169 1.409l5-1.429A1 1 0 009 15.571V11a1 1 0 112 0v4.571a1 1 0 00.725.962l5 1.428a1 1 0 001.17-1.408l-7-14z" />
                        </svg>Submit Request</button>
                    <button type="reset" class="text-white bg-gray-600 hover:bg-gray-700 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-base px-8 py-3.5 inline-flex items-center justify-center min-w-[200px] shadow-md hover:shadow-lg transition"><svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M4 2a1 1 0 011 1v2.101a7.002 7.002 0 0111.601 2.566 1 1 0 11-1.885.666A5.002 5.002 0 005.999 7H9a1 1 0 010 2H4a1 1 0 01-1-1V3a1 1 0 011-1zm.008 9.057a1 1 0 011.276.61A5.002 5.002 0 0014.001 13H11a1 1 0 110-2h5a1 1 0 011 1v5a1 1 0 11-2 0v-2.101a7.002 7.002 0 01-11.601-2.566 1 1 0 01.61-1.276z" clip-rule="evenodd" />
                        </svg>Reset Form</button>
                </div>
            </form>
        </div>
    </div>
    <script>
        function toggleDateFields(select) {
            ...
            const isInd = table.id === 'individualTable';
            const startCell = row.querySelector('.start-date').closest('td');
            const endCell = row.querySelector('.end-date').closest('td');
            const start = row.querySelector('.start-date');
            const end = row.querySelector('.end-date');
            const isTemp = select.value === 'temporary';
            if (isTemp) {
                startCell.classList.remove('hidden');
                endCell.classList.remove('hidden');
                ...
                startCell.classList.add('hidden');
                endCell.classList.add('hidden');
                start.disabled = true;
                end.disabled = true;
        ...
        function addRow(tableId) {
            ...
            if (tableId === 'individualTable') {
                ...
                const usernameInput = lastRow.querySelector('input[name="ind_user_names"]') || lastRow.querySelector('input[name="ind_username_copy[]"]');
                ...
                if (!username || !application || !accessType || !durationType || !dateNeeded || !justification) {
                    Swal.fire({
                        ...
                        text: 'Fill all fields before adding a new row.',
                ...
                if (durationType === 'temporary' && (!startDate || !endDate)) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'Missing Dates',
                        text: 'Set start and end dates for temporary access.',
                        confirmButtonColor: '#0084FF'
                    });
                    return;
                }
                const newRow = document.createElement('tr');
                newRow.innerHTML = `<td class="border border-gray-200 p-2"><input type="hidden" name="ind_username_copy[]" value="${username}"><div class="invisible">Username</div></td><td class="border border-gray-200 p-2"><select name="ind_application[]" class="w-full p-2 border border-gray-300 rounded" required><option value="">Select Application</option><?php foreach ($systemApplications as $app): ?><option value="<?php echo $app; ?>"><?php echo $app; ?></option><?php endforeach; ?></select><input type="hidden" name="ind_application_system[]" value=""></td><td class="border border-gray-200 p-2"><select name="ind_access_type[]" class="w-full p-2 border border-gray-300 rounded" required><option value=""></option><option value="full">Full</option><option value="read">Read</option><option value="admin">Admin</option></select></td><td class="border border-gray-200 p-2"><select name="ind_duration_type[]" class="duration-select w-full p-2 border border-gray-300 rounded" required onchange="toggleDateFields(this)"><option value=""></option><option value="permanent">Permanent</option><option value="temporary">Temporary</option></select></td><td class="border border-gray-200 p-1 hidden"><input type="date" name="ind_start_date[]" class="start-date w-full p-1 border border-gray-300 rounded" disabled></td><td class="border border-gray-200 p-1 hidden"><input type="date" name="ind_end_date[]" class="end-date w-full p-1 border border-gray-300 rounded" disabled></td><td class="border border-gray-200 p-1"><input type="date" name="ind_date_needed[]" class="date-needed w-full p-1 border border-gray-300 rounded" required></td><td class="border border-gray-200 p-2"><input type="text" placeholder="Click to add justification" name="ind_justification[]" class="justification-input w-full p-2 border border-gray-300 rounded bg-gray-50 cursor-pointer" required></td><td class="border border-gray-200 p-2 text-center w-16"><button type="button" class="bg-red-600 hover:bg-red-700 text-white p-2 rounded" onclick="deleteRow(this)"><i class="fa fa-trash"></i></button></td>`;
                tbody.appendChild(newRow);
                setupJustificationInputs();
                const ds = newRow.querySelector('.duration-select');
                if (ds) {
                ...
                if (!application || !username || !accessType || !durationType || !dateNeeded || !justification) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'Complete Current Row',
                        text: 'Fill all fields in the current application row before adding a new application.',
                        confirmButtonColor: '#0084FF'
                    });
                    return;
                }
                ...
                newRow.innerHTML = `<td class="border border-gray-200 p-2"><div class="flex gap-2 items-center"><select name="grp_application[]" class="app-select flex-1 p-2 border border-gray-300 rounded" required><option value="">Select Application</option><?php foreach ($systemApplications as $app): ?><option value="<?php echo $app; ?>"><?php echo $app; ?></option><?php endforeach; ?></select><input type="hidden" name="grp_application_system[]" value=""></div></td><td class="border border-gray-200 p-2"><div class="flex items-center gap-2"><input type="text" placeholder="User Name" name="grp_user_names[]" class="w-full p-2 border border-gray-300 rounded" required><button type="button" class="bg-blue-600 hover:bg-blue-700 text-white p-2 rounded" onclick="addUserRow(this)" title="Add User"><i class="fa fa-user-plus"></i></button></div></td><td class="border border-gray-200 p-2"><select name="grp_access_type[]" class="w-full p-2 border border-gray-300 rounded" required><option value=""></option><option value="full">Full</option><option value="read">Read</option><option value="admin">Admin</option></select></td><td class="border border-gray-200 p-2"><select name="grp_duration_type[]" class="duration-select w-full p-2 border border-gray-300 rounded" required onchange="toggleDateFields(this)"><option value=""></option><option value="permanent">Permanent</option><option value="temporary">Temporary</option></select></td><td class="border border-gray-200 p-1 hidden"><input type="date" name="grp_start_date[]" class="start-date w-full p-1 border border-gray-300 rounded" disabled></td><td class="border border-gray-200 p-1 hidden"><input type="date" name="grp_end_date[]" class="end-date w-full p-1 border border-gray-300 rounded" disabled></td><td class="border border-gray-200 p-1"><input type="date" name="grp_date_needed[]" class="date-needed w-full p-1 border border-gray-300 rounded" required></td><td class="border border-gray-200 p-2"><input type="text" placeholder="Click to add justification" name="grp_justification[]" class="justification-input w-full p-2 border border-gray-300 rounded bg-gray-50 cursor-pointer" required></td><td class="border border-gray-200 p-2 text-center w-16"><button type="button" class="bg-red-600 hover:bg-red-700 text-white p-2 rounded" onclick="deleteRow(this)"><i class="fa fa-trash"></i></button></td>`;
                tbody.appendChild(newRow);
                setupJustificationInputs();
                const ds = newRow.querySelector('.duration-select');
                if (ds) {
        ...
        function addUserRow(button) {
            ...
            if (durationType === 'temporary') {
                const sd = appRow.querySelector('input[name="grp_start_date[]"]').value;
                const ed = appRow.querySelector('input[name="grp_end_date[]"]').value;
                if (!sd || !ed) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'Missing Dates',
            ...
            newRow.innerHTML = `<td class="border border-gray-200 p-2 bg-gray-50">${selectedApp}<input type="hidden" name="grp_application[]" value="${selectedApp}"><input type="hidden" name="grp_application_system[]" value="${selectedApp}"></td><td class="border border-gray-200 p-2"><input type="text" placeholder="User Name" name="grp_user_names[]" class="w-full p-2 border border-gray-300 rounded" required></td><td class="border border-gray-200 p-2"><select name="grp_access_type[]" class="w-full p-2 border border-gray-300 rounded" required><option value="">Select Access Type</option><option value="full">Full</option><option value="read">Read</option><option value="admin">Admin</option></select></td><td class="border border-gray-200 p-2"><select name="grp_duration_type[]" class="duration-select w-full p-2 border border-gray-300 rounded" required onchange="toggleDateFields(this)"><option value="">Select Duration</option><option value="permanent">Permanent</option><option value="temporary">Temporary</option></select></td><td class="border border-gray-200 p-1 hidden"><input type="date" name="grp_start_date[]" class="start-date w-full p-1 border border-gray-300 rounded" disabled></td><td class="border border-gray-200 p-1 hidden"><input type="date" name="grp_end_date[]" class="end-date w-full p-1 border border-gray-300 rounded" disabled></td><td class="border border-gray-200 p-1"><input type="date" name="grp_date_needed[]" class="date-needed w-full p-1 border border-gray-300 rounded" required></td><td class="border border-gray-200 p-2"><input type="text" placeholder="Click to add justification" name="grp_justification[]" class="justification-input w-full p-2 border border-gray-300 rounded bg-gray-50 cursor-pointer" required></td><td class="border border-gray-200 p-2 text-center w-16"><button type="button" class="bg-red-600 hover:bg-red-700 text-white p-2 rounded" onclick="deleteRow(this)"><i class="fa fa-trash"></i></button></td>`;
            appRow.after(newRow);
            setupJustificationInputs();
            const ds = newRow.querySelector('.duration-select');
            if (ds) {
        ...
        function setupApplicationSelects() {
            document.querySelectorAll('select[name="ind_application[]"]').forEach(function(s, i) {
                s.addEventListener('change', function() {
                    const h = document.getElementsByName('ind_application_system[]')[i];
                    if (h) h.value = this.value;
                });
            });
            document.querySelectorAll('select[name="grp_application[]"]').forEach(function(s, i) {
                s.addEventListener('change', function() {
                    const h = document.getElementsByName('grp_application_system[]')[i];
                    if (h) h.value = this.value;
                });
            });
        }
        ...
        function setupJustificationInputs() {
            document.querySelectorAll('.justification-input').forEach(inp => {
                ...
                inp.addEventListener('click', function(e) {
                    ...
                    Swal.fire({
                        ...
                        input: 'textarea',
                        inputLabel: 'Provide detailed justification',
                        ...
                        if (res.isConfirmed && res.value) ci.value = res.value;
                    });
                });
            });
        }
        ...
        $(function() {
            ...
            $('#uarForm').on('submit', function(e) {
                ...
                fd.append('requestor_name', name);
                fd.append('company', company);
                fd.append('department', department);
                fd.append('request_date', date);
                fd.append('employee_id', <?php echo json_encode($requestorId); ?>);
                fd.append('employee_email', <?php echo json_encode($_SESSION['employee_email'] ?? ($info['employee_email'] ?? '')); ?>);
                const userForms = [];
                ...
                if (userForms.length === 0) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'Missing Information',
                        text: 'Add at least one valid access request.',
                        confirmButtonColor: '#0084FF'
                    });
                    return;
                }
                if (!name || !company || !department || !date) {
                    Swal.fire({
                        ...
                        text: 'Complete Requestor Information.',
                        confirmButtonColor: '#0084FF'
                    });
                    return;
                }
                fd.append('user_forms', JSON.stringify(userForms));
                ...
                $.ajax({
                    ...
                    if (r.success) {
                        let reqNo = '';
                        if (r.message && r.message.includes('Request number is')) {
                            const m = r.message.match(/Request number is (\d+-\d+)/);
                            if (m && m[1]) reqNo = m[1];
                        }
                        Swal.fire({
                            icon: 'success',
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
                            text: r.message || 'Submission failed.',
                            confirmButtonColor: '#0084FF'
                        });
                    }
                }).fail(function(xhr) {
